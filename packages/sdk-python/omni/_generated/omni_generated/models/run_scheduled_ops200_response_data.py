...
class RunScheduledOps200ResponseData(BaseModel):
    """
    RunScheduledOps200ResponseData
    """ # noqa: E501
    dead_letter_retry: RunScheduledOps200ResponseDataDeadLetterRetry = Field(alias="deadLetterRetry")
    payload_cleanup: RunScheduledOps200ResponseDataPayloadCleanup = Field(alias="payloadCleanup")
    __properties: ClassVar[List[str]] = ["deadLetterRetry", "payloadCleanup"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of dead_letter_retry
        if self.dead_letter_retry:
            _dict['deadLetterRetry'] = self.dead_letter_retry.to_dict()
        # override the default output from pydantic by calling `to_dict()` of payload_cleanup
        if self.payload_cleanup:
            _dict['payloadCleanup'] = self.payload_cleanup.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "deadLetterRetry": RunScheduledOps200ResponseDataDeadLetterRetry.from_dict(obj["deadLetterRetry"]) if obj.get("deadLetterRetry") is not None else None,
            "payloadCleanup": RunScheduledOps200ResponseDataPayloadCleanup.from_dict(obj["payloadCleanup"]) if obj.get("payloadCleanup") is not None else None
        })
        return _obj
