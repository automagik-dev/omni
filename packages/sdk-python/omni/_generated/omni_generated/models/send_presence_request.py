...
class SendPresenceRequest(BaseModel):
    ...
    to: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Chat ID to show presence in")
    type: StrictStr = Field(description="Presence type")
    duration: Optional[Annotated[int, Field(le=30000, strict=True, ge=0)]] = Field(default=5000, description="Duration in ms before auto-pause (default 5000, 0 = until paused)")
    __properties: ClassVar[List[str]] = ["instanceId", "to", "type", "duration"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['typing', 'recording', 'paused']):
            raise ValueError("must be one of enum values ('typing', 'recording', 'paused')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "to": obj.get("to"),
            "type": obj.get("type"),
            "duration": obj.get("duration") if obj.get("duration") is not None else 5000
        })
        return _obj
