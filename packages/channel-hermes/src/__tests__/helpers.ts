/**
 * Shared test doubles for the Hermes plugin suite — mirrors the
 * MockEventBus/MockLogger/MockStorage pattern from channel-twilio-whatsapp.
 */
...
export const instanceId = '00000000-0000-4000-8000-000000000002';
export const MEDIA_ID = '11111111-2222-4333-8444-555555555555';
...
/**
 * Connect the plugin. The caller must have mocked `fetch` so the sign_in
 * performed by `client.ping()` resolves (e.g. `jsonResponse({ jwt: 'jwt-1' })`).
 */
export async function connectPlugin(plugin: HermesPlugin): Promise<void> {
  await plugin.connect(instanceId, {
    instanceId,
    credentials: {
      hermesBaseUrl: 'https://hermes.example.com',
      hermesUsername: 'omni-user',
      hermesPassword: 'omni-pass',
      hermesMediaId: MEDIA_ID,
      hermesTemplateNamespace: 'a1b2c3d4_name_space',
    },
  });
}
