...
**Investigation path:**
1. Initially suspected system-wide issue on instance — WRONG
2. Checked prod DB: Gabriel Moojen has 13 incoming msgs on SAME instance today — pipeline works
3. Found root cause in PM2 logs: **Bad MAC** decryption errors at libsignal layer
4. All 4 messages from Chiozzini today fail `verifyMAC` → never reach `messages.upsert`
...
**Also found:** `getMessage` callback not provided to `makeWASocket()` — returns `undefined` by default. Doesn't cause the issue but limits retry effectiveness.
...
|-----|----------|---------------|
| `GROQ_API_KEY` | Audio transcription (Whisper) | ❌ Missing both envs |
| `OPENAI_API_KEY` | Audio fallback + Image vision fallback | ❌ Missing both envs |
| `GEMINI_API_KEY` | Image/Video/Doc vision (primary) | ❌ Missing everywhere |
...
5. Verified startup logs: `hasGroq: true, hasOpenAI: true, hasGemini: true` ✅

**Important PM2 lesson:** `pm2 restart` alone doesn't reload env vars from `.env` — Bun reads `.env` at process start, so the restart DID work, but `pm2 env` only shows PM2-managed vars, not what Bun loaded internally.

## ClawNet Update (from Guga)

Guga deployed ClawNet — cross-gateway agent messaging. Saved full connection details to MEMORY.md:
- cegonha 🦩, juice 🧃 (Jerry), guardiola (outbound), luis (outbound)
...
- Documents (PDF/DOCX/XLSX extraction, OCR, MD→PDF, HTML→PDF)
- Bidirectional conversions (audio↔text, image↔text, PDF↔text, etc.)
- AI/Analysis (sentiment, NER, language detection, translation, embeddings)

**Providers mapped by cost tier:**
- 💚 Cheap: Groq, Gemini Flash, Deepgram, DeepL, Local (FFmpeg, pdf-parse, etc.)
- 🟡 Medium: OpenAI, ElevenLabs, Anthropic, Stability AI
...
Sub-agent ("tentáculo") came back with the fix ready. I executed it live on prod:

1. **Confirmed** 3 Signal sessions for Chiozzini in `plugin_storage` (`:0`, `:52`, `:54`) — `:54` was the corrupted one
2. **Executed** `DELETE FROM plugin_storage WHERE key = '...session:63273542144196_1.54'` → `DELETE 1` ✅
3. **Restarted** `pm2 restart omni-v2-api` → online (pid 511094, 200MB)
...
- **Not yet executed** — waiting Felipe's go-ahead (it's his money)

## Files Updated
- HEARTBEAT.md — removed done items, added real blockers
- MEMORY.md — corrected open items, added audit table, added ClawNet + Chiozzini + API keys sections
- 6 wish files — status corrections
- `.env` (dev) — added GROQ, OPENAI, GEMINI keys
- Prod `.env` (CT 140) — added same keys
- Beads `omni-td4` — new epic for media pipeline v2
...
### Wishes
- Phase 1 wish (APPROVED): `.wishes/openclaw-provider-integration/openclaw-provider-integration-wish.md` (Groups 0–C only)
- Phase 2 wish (DRAFT): `.wishes/session-observatory/session-observatory-wish.md` with council “must-fix” list
...
### Independent 3-way review (Opus tentacles) — all **MODIFY**
...
3. **Schema/wiring:** `packages/db/src/schema.ts` still has duplicated `providerSchemas` const (4th schema source of truth) — must derive or satisfy-check against `@omni/core` `PROVIDER_SCHEMAS`; CLI missing TLS/cert hint; minor JSDoc mismatch.
...
- Investigated “slow lint”: Biome lint is fast (~0.4s). The pain was `make typecheck` (uncached ~22s) dominated by `@omni/api` heavy TS generics (1.4M instantiations).
- Shipped to `dev`: enable TypeScript incremental builds (`incremental: true` in root tsconfig). Warm typecheck improved ~22s → ~10s (no turbo cache), and ~22s → ~4.7s after a single api file edit (turbo partial cache).
...
- **Do NOT run `claude` directly** for repo review here: it prompted **“Not logged in · Please run /login”**.
- Use **`claudio launch main -- --dangerously-skip-permissions -p "..."`** to route via Juice + avoid permissions babysitting.
- `process log` for the background `claudio` session often **doesn’t stream output** (PTY/subprocess). Workaround:
...
- Worktree gotcha: can’t create a worktree from a branch already checked out; used a **temp clone** of the local repo instead.
...
2. **`rawEvent` type mismatch** in `packages/api/src/plugins/agent-dispatcher.ts` (casts `firstMessage.payload` to `AgentTrigger['event']` via `as unknown as` even though it’s not an `OmniEvent`). Will break downstream consumers and future trigger_logs.
